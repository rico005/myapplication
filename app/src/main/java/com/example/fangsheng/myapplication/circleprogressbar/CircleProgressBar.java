package com.example.fangsheng.myapplication.circleprogressbar;
...
/**
 * Created by fangsheng on 2017/5/25.
 */
...
public class CircleProgressBar extends View
{
     ...
     * 当前进度值
     */
    private int currentValue = DEFAULT_PERCENT;
    ...
    /**
     * 起始角度(x轴右边横向作为0度起始点，例如从顶上开始就是-90度)
     */
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            switch (attr)
            {
                ...
                case R.styleable.circleProgressBar_startAngle:
                    startAngle = ta.getFloat(attr, -360);
                    break;
                case R.styleable.circleProgressBar_angleRange:
                    angleRange = ta.getFloat(attr, 360);
                default:
                    break;
            }
        }
    ...
    /**
     * 通过startAngle和angleRange来计算出合适的最终startAngle值
     * @return
     */
    private float getStartAngle(){
        if (startAngle > -360){
            ...
            //通过angleRange自动计算startAngle，左右均分angleRange，开口朝下
            return  -(angleRange / 2 + 90);
        }
    }

    /**
     * 如果弧度小于360度，需要对colorArray做特殊处理
     * @return
     */
    private int[] getColorArray(){
        if (angleRange < 360){ //如果不是完整的圆，需要特殊处理将颜色值分size+1段，让最后一段开头就达到endColor的值，因为最后一段不会显示
            int size = colorArray.length;
            int[] colorArrayNew = new int[size + 1];
            for (int i=0 ; i<size; i++){
                colorArrayNew[i] = colorArray[i];
            }
            colorArrayNew[size] = colorArray[size - 1];
            return colorArrayNew;
        }else {
            return colorArray;
        }
    }
    ...
    private float[] getPositions(){
        if (angleRange < 360){ //如果不是完整的圆，需要特殊处理将颜色值分size+1段，让最后一段开头就达到endColor的值，因为最后一段不会显示
            int size = colorArray.length;
            float[] positions = new float[size + 1];
            positions[0] = 0;
            for (int i=1; i<size; i++){
                positions[i] = angleRange * (i+1) * 1.0f / size * 1.0f / 360f;
            }
            positions[size] = 1;
            return positions;
...
//        canvas.drawCircle(center, center, radius, circlePaint); // 画底部的空心圆
        ...
        canvas.drawArc(oval, getStartAngle(), angleRange, false, circlePaint);
...
//                - circleWidth, getMeasuredHeight() - circleWidth, colorArray, null, Shader.TileMode.MIRROR);
        ...
        SweepGradient sweepGradient = new SweepGradient(center, center, getColorArray(), getPositions());
        Matrix matrix = new Matrix();
        matrix.setRotate(getStartAngle() - 5, center, center);//加上旋转还是很有必要的，每次最右边总是有一部分多余了,不太美观,加上-5调整下
...
//        circlePaint.setShadowLayer(10, 10, 10, Color.RED);
        ...
        alphaAngle = currentValue * angleRange / maxValue * 1.0f; // 计算每次画圆弧时扫过的角度，这里计算要注意分母要转为float类型，否则alphaAngle永远为0
        canvas.drawArc(oval, getStartAngle(), alphaAngle, false, circlePaint);
    }
